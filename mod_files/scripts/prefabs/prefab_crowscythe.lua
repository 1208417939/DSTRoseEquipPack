...
local data_cfg = weapon_data_resolver.resolve_weapon_data("crowscythe")
if data_cfg == nil then
    error("Failed to resolve weapon data for crowscythe")
end

local weapon_factory = require("rose_core/rose_weapon_factory")
local crowscythe_def = require("rose_defs/weapon_def_builder").build("crowscythe")
...
local nightvision_dirty_event = "crowscythe_nightvision_dirty"
...
local function refresh_local_player_vision(inst)
    if ThePlayer == nil then
        return
    end

    if inst.replica == nil or inst.replica.inventoryitem == nil or inst.replica.equippable == nil then
        return
    end
    ...
    ThePlayer:PushEvent("equip", {
        item = inst,
        eslot = EQUIPSLOTS.HANDS,
    })
end

local function on_nightvision_dirty(inst)
    local enabled = inst._nightvision_enabled ~= nil and inst._nightvision_enabled:value() or false
    sync_nightvision_tag(inst, enabled)
    refresh_local_player_vision(inst)
end
...
local function fn()
    ...
    if not TheWorld.ismastersim then
        inst:ListenForEvent(nightvision_dirty_event, on_nightvision_dirty)
        inst:DoTaskInTime(0, on_nightvision_dirty)
        return inst
    end
    ...
    weapon_factory.attach_runtime(inst, crowscythe_def)
